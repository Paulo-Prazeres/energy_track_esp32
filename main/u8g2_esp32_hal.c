...
uint8_t u8g2_esp32_i2c_byte_cb(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr) {
	...
	switch(msg) {
		...
		case U8X8_MSG_BYTE_SEND: {
			...
			ESP_LOGD(TAG, "U8X8_MSG_BYTE_SEND");
...
uint8_t u8g2_esp32_gpio_and_delay_cb(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr) {
	...
	// specified then we define those pins as GPIO outputs.
		case U8X8_MSG_GPIO_AND_DELAY_INIT: {
			...
			/*gpio_pad_select_gpio(PROXIMO_PINO);
    		gpio_set_direction(PROXIMO_PINO, GPIO_MODE_INPUT);
    		gpio_pulldown_dis(PROXIMO_PINO);
    		// gpio_pullup_en(PIN_SWITCH);
    		gpio_pullup_en(PROXIMO_PINO);
			gpio_intr_disable(PROXIMO_PINO);

			gpio_pad_select_gpio(PREVIO_PINO);
    		gpio_set_direction(PREVIO_PINO, GPIO_MODE_INPUT);
    		gpio_pulldown_dis(PREVIO_PINO);
    		// gpio_pullup_en(PIN_SWITCH);
    		gpio_pullup_en(PREVIO_PINO);
			gpio_intr_disable(PREVIO_PINO);

			gpio_pad_select_gpio(SELETOR_PINO);
    		gpio_set_direction(SELETOR_PINO, GPIO_MODE_INPUT);
    		gpio_pulldown_dis(SELETOR_PINO);
    		// gpio_pullup_en(PIN_SWITCH);
    		gpio_pullup_en(SELETOR_PINO);
			gpio_intr_disable(SELETOR_PINO);

			gpio_pad_select_gpio(CASA_PINO);
    		gpio_set_direction(CASA_PINO, GPIO_MODE_INPUT);
    		gpio_pulldown_dis(CASA_PINO);
    		// gpio_pullup_en(PIN_SWITCH);
    		gpio_pullup_en(CASA_PINO);
			gpio_intr_disable(CASA_PINO);*/
...
//				printf("%c",(arg_int==1?'D':'d'));
		...
		/*case U8X8_MSG_GPIO_MENU_SELECT:
      			u8x8_SetGPIOResult(u8x8, gpio_get_level(SELETOR_PINO));
      		break;				

		case U8X8_MSG_GPIO_MENU_NEXT:
      			u8x8_SetGPIOResult(u8x8, gpio_get_level(PROXIMO_PINO));
      		break;
		
		case U8X8_MSG_GPIO_MENU_PREV:
      			u8x8_SetGPIOResult(u8x8, gpio_get_level(PREVIO_PINO));
      		break;

    	case U8X8_MSG_GPIO_MENU_HOME:
      			u8x8_SetGPIOResult(u8x8, gpio_get_level(CASA_PINO));
      		break;*/
