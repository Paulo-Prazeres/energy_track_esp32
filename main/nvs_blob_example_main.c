...
static const char *NVS_TAG = "NVS_TAG";
...
void execute_schedule_action()
{
    ...
    EventBits_t bits = xEventGroupWaitBits(sntp_event_group,
            SNTP_CONNECTED_BIT | SNTP_FAIL_BIT,
            pdFALSE,
            pdFALSE,
            portMAX_DELAY);

    if (bits & SNTP_CONNECTED_BIT) ESP_LOGI(NVS_TAG, "SNTP got date successfully! Starting task to check time and dates for schedules...");
    ...
    while(1)
    {
        ...
        {            
            ...
            {
                if(time_now == loadsEventsLoaded[i].eventsON[j].date) 
                {
                    printf("Turning ON load %s at %ld\n", loadsEventsLoaded[i].loadName, time_now);
                    err = delete_or_change_sched_from_NVS(loadsEventsLoaded[i].loadName, time_now, _NULL, 0);
                    if(err == ESP_OK) printf("Time %ld deleted from NVS.\n", time_now);
                    else printf("Not possible to delete from NVS.\n");
                }
            }
            for(int j = 0; j < loadsEventsLoaded[i].numOfEventsOFF; j++)
            {
                if(time_now == loadsEventsLoaded[i].eventsOFF[j].date) 
                {
                    printf("Turning OFF load %s at %ld\n", loadsEventsLoaded[i].loadName, time_now);
                    err = delete_or_change_sched_from_NVS(loadsEventsLoaded[i].loadName, time_now, _NULL, 0);
                    if(err == ESP_OK) printf("Time %ld deleted from NVS.\n", time_now);
                    else printf("Not possible to delete from NVS.\n");
                }
